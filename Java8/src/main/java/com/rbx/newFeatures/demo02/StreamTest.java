...
public class StreamTest {
    ...
    public static void arr5() {
        ...
        list.add(new Apple(103, "苹果3", "绿色"));
        list.add(new Apple(102, "苹果2", "绿色"));

        System.out.println("1111");
        //list.stream().sorted(Comparator.comparing(Apple::getNum, (o1, o2) -> o2 - o1)).forEach(apple -> System.out.println(apple.getNum()+"-"+apple.getName()));
        List<Apple> collect2 = list.stream().sorted(Comparator.comparing(Apple::getNum)).collect(Collectors.toList());
        collect2.stream().forEach(apple -> System.out.println(apple.getNum()+"-"+apple.getName()));
        //将流中内容收集到Map集合中
        Map<Integer, String> collect3 = list.stream().collect(Collectors.toMap(Apple::getNum, Apple::getName));
        collect3.forEach((integer, s) -> System.out.println(integer+"-"+collect3.get(integer)));
        //收集流中Integer属性的统计值
        IntSummaryStatistics collect4 = list.stream().collect(Collectors.summarizingInt(Apple::getNum));
        System.out.println("collect4.getAverage() = " + collect4.getAverage());
        System.out.println("collect4.getCount() = " + collect4.getCount());
        System.out.println("collect4.getMax() = " + collect4.getMax());
        System.out.println("collect4.getMin() = " + collect4.getMin());
        System.out.println("collect4.getSum() = " + collect4.getSum());
        //joining连接  delimiter分隔符 , prefix前缀 , suffix后缀
        String collect5 = list.stream().map(Apple::getName).collect(Collectors.joining("---","(",")"));
        System.out.println(collect5);
        //groupingBy进行分组  根据颜色分组
        Map<String, List<Apple>> collect6 = list.stream().collect(Collectors.groupingBy(Apple::getColor));
        collect6.forEach((s, apples) -> System.out.println(s+"-"+collect6.get(s)));
        //groupingBy分组 , 先根据颜色分组 , 在根据Num是否小于<105进行分组
        Map<String, Map<Boolean, List<Apple>>> collect7 = list.stream().collect(Collectors.groupingBy(Apple::getColor, Collectors.groupingBy(x -> x.getNum() < 105)));
        ...
        Map<Boolean, List<Integer>> mapS = listA.stream()
                .collect(Collectors.partitioningBy(num -> num % 2 == 0));
        System.out.println(mapS);
        Map<Boolean, List<Integer>> mapSS = listA.stream().collect(Collectors.groupingBy(num -> num % 2 == 0));
        System.out.println(mapSS);

        //collectingAndThen(Collector<T, A, R> downstream, Function<R, RR> finisher)
        //downstream参数是一个Collector对象，用于指定如何收集元素；finisher参数是一个Function对象，用于在收集完成后对结果进行转换。
